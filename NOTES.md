...
- HeroIllustration調整2件＋`/faq`ページ新規作成：
  - 【発見した既存バグ】Header・Footer・`/contact`・`/how-it-works`から`/faq`へのリンクが既に置かれていたが、`src/app/faq/page.tsx`自体が存在せず404になっていた。Q&A6件（申込みの流れ／キャンセル方法／承認までの期間／支払いトラブル時の対応／Discordを使わない場合／利用手数料）を`<details>`によるアコーディオン形式で新規作成し解消。内容は実際のアプリ仕様（Player側にキャンセル機能が無いこと、決済に非関与であること等）に合わせた
  - `HeroIllustration.tsx`のPC版（lg以上）で表示していた、マッチング成立の円の左右の横向き矢印（Arrowコンポーネント）を削除（未使用になったArrow関数自体も削除）
  - `HeroIllustration.tsx`のUserカードのサンプル名を「けんじ」→「ひらく」に変更
...
- 集客開始前の修正4件のうち②「ご利用の流れ」ページの修正対応（`src/app/how-it-works/page.tsx`）：
  - 【実装確認で判明した認識ズレ】支払い条件（前払い/後払い・支払い手段）は出品(`creator_games`)ごとではなく、Creatorのプロフィール1つに対して設定し全出品に共通適用される方式（詳細は上記「Creatorの支払い条件機能」の項目を参照）。当初案にあった「出品を作る際にあわせて支払い条件を設定」という手順は実態と異なるため、「登録する」の手順に支払い条件設定を含める形に修正して反映
  - 冒頭にCreator/Playerの役割説明ブロックを追加（「同じアカウントで両方できます」の一文は、出品1件目の作成でDBトリガーがprofiles.is_creatorを自動trueにする実装＝別アカウント種別が存在しないことを確認した上でそのまま採用）
  - Creator/Playerそれぞれの流れを7ステップに更新し、前払い/後払いのケースを【前払いの場合】【後払いの場合】として明示（支払いを最後の1ステップに固定していた旧構成を解消）
  - ページ下部の「お支払いについて」ボックス（β版なので直接支払い／将来的に決済機能を提供予定、という文言）を、支払い条件の設定単位・振込先の表示タイミング・手数料0円である旨を明記する内容に置き換え
  - 前払い/後払いの分岐表示については、Creator詳細ページ・申込ページが既に「設定されている値のみ」を単一Badgeで表示する実装のため追加対応不要と判断（現状維持）
  - サイト全体で「ユーザー」表記がPlayerの意味に使われている箇所を洗い出し、`src/lib/queries/admin.ts`の管理ダッシュボード用フォールバック文言`不明なユーザー`（申込者=Player名の欠損時表示、隣接する`不明なCreator`と不整合だった）のみを`不明なPlayer`に修正。他（bookings.tsの相手方汎用フォールバック、Creator通報リンクの「このユーザー」、privacy/terms内の定義語としての「ユーザー」、admin画面の「登録ユーザー総数」）はPlayer限定の意味ではないため対象外と判断し変更なし
- 集客開始前の修正4件のうち③「利用規約の追記」対応（`src/app/terms/page.tsx`）：
  - 【実装確認の結果】年齢制限（18歳以上限定）は既に第2条（利用資格・年齢制限）に明記済みだったため追記不要と判断（運営側の記憶通り実装済み）
  - キャンセル時の原則は未記載だったため、第5条（料金および支払い）の直後に新設の第6条（キャンセルについて）を追加：前日までのキャンセルは無償、当日は当事者間で協議、GameMatchは決済を仲介しないため返金には関与しない、の3点を明記。挿入に伴い旧第6条〜第13条を第7条〜第14条に繰り下げ
  - 番号ズレに伴い、`src/app/creators/[id]/request/page.tsx`内の支払い注意喚起コメント（旧「利用規約 第5条・第8条と整合」）を新番号（第5条・第6条・第9条）に更新。NOTES.md内の過去の記載（本項目より前の「第5条・第8条」等の記述）はその時点の履歴として残し、遡って書き換えはしていない
- 集客開始前の修正4件のうち④「運営の対応時間帯の明示」対応：運営（海外在住・本業あり）が即時対応できないことによる期待値のズレを防ぐ目的
  - `src/app/faq/page.tsx`：新規Q&A「運営からの返信はどのくらいで来ますか？」を追加。「運営からのご連絡は、毎日20時〜24時(日本時間)にまとめて対応しています」の旨を明記
  - 【あわせて発見・修正】`src/app/contact/page.tsx`の導入文に「できる限り迅速に対応いたします。」という、上記の対応時間帯と矛盾する文言があったため、同じ対応時間帯の案内に差し替え（期待値のズレ防止という今回の趣旨に沿って対応）
