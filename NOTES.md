...
### すぐ直す（今週）
- 重複Aの整理：上部の「推し活/コーチング」2カードと、その下のフィルター（一緒に遊ぶ/コーチング/ランクアップ支援/VTuber交流）が内容重複。整理方針は検討中（案1: 2カードをフィルター連動の入口ボタン化＋下フィルターの重複ラベル整理 / 案2: 2カードは世界観紹介・フィルターは検索機能と役割分離）
- 重複Bの整理：Heroの3点（手数料0円/連絡先非公開/登録無料）と、下部の安全性3点（個人情報非公開/サポート/登録無料）が「連絡先非公開」「登録無料」で重複。Heroは「強み・気軽さ」、下部は「安全・安心の詳細」に役割分離して重複解消
- 「かんたん3ステップ」の命名変更：何の流れか不明瞭。「ゲームを一緒に始めるまでの流れ」等、結果が伝わる命名に
...
### 補足
- 「受け取った申込／申込状況」がログイン時のヘッダーにのみ表示されるのは、本人だけが見るべき個人情報のため意図的な正しい設計（変更不要）
